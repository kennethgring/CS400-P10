...
public class DeliveryQueueTester {
  public static boolean testOfferDelivery() {
    ...
    if(minHeap.getSize() != 4) { System.out.println("test1 fail"); return false;}
    // check first (highest priority delivery to be returned)
    String bestDelivery = minHeap.pollBestDelivery().toString();
    if(!bestDelivery.equals("The distance between 2 and A is 2")) { System.out.println("test2 fail" + bestDelivery); return false;}
    // check if the size is correct (only delivery w/student 1 + robot B left)
    if(minHeap.getSize() != 1) { System.out.println("test3 fail"); System.out.println(minHeap.getSize());return false;}
    // check last (lowest priority delivery to be returned)
    String worstDelivery = minHeap.peek().toString();
    if(!worstDelivery.equals("The distance between 1 and B is 7")) { System.out.println("test4 fail"); return false;}

    // only return true after all previous tests pass
    return true;
}
  public static void main(String[] args) {
    if (testOfferDelivery()) {
      System.out.println("testOfferDelivery() works");
    }
    else {
      System.out.println("testOfferDelivery() failed");
    }
  }
}
